...
| `cineharbor-worker` | 边缘代理 + 下载 Service Worker（P6） | ✅ P6 迁入 |
| `cineharbor-download-site` | 下载站（P6） | ✅ P6 迁入 |
...
| **P4 Web 客户端** | 旧 Next.js 应用迁入 cineharbor-web，清旧品牌；数据访问切 core 的 /addons 聚合 | cineharbor-web 起服务 | ✅ 迁入+build✅ + addon 传输层✅ + live/douban/vod 内置 addon✅；前端切面收敛到 local-service 已定为终态（ADR-0005 取代 ADR-0004 长尾条款），切面进行中 |
...
| **P6 周边** | worker、proxy、下载站迁入改名；logo 冻结后换 favicon/icons | 品牌一致 | ✅ worker+下载站迁入 + 品牌资产（字标/favicon/PWA/桌面图标）全部完成 |
